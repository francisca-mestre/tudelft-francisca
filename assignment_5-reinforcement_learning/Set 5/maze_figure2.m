...
actions = cell(1,63);
for s = 1:63
    if pi(s) == 1 && ~ismember(s,grey_states)
        actions{s} = '\leftarrow';
        text(centersx(s),centersy(s),actions{s},'fontsize',12,'color','b');
    elseif pi(s) == 2 && ~ismember(s,grey_states)
        actions{s} = '\rightarrow';
        text(centersx(s),centersy(s),actions{s},'fontsize',12,'color','b');
    elseif pi(s) == 3 && ~ismember(s,grey_states)
        actions{s} = '\uparrow';
        text(centersx(s),centersy(s),actions{s},'fontsize',12,'color','b');
    elseif pi(s) == 4 && ~ismember(s,grey_states)
        actions{s} = '\downarrow';
        text(centersx(s),centersy(s),actions{s},'fontsize',12,'color','b');
    end
end
