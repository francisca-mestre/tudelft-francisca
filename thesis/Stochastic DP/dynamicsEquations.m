...
dynSoC = @(s1,s2,s3,u,z) s1+max(u,0)*rateCharge+min(u,0)*rateDischarge;  % max(0,min(1,s1+max(u,0)*rateCharge+min(u,0)*rateDischarge));
dynPeak = @(s1,s2,s3,u,z) max(minPeak, min(maxPeak, max(0, max(s2,(s3+z+u)))));  % min(maxPeak,max(0,max(s2,(s3+z+u))));
dynLoad = @(s1,s2,s3,u,z) max(minPeak, min(maxPeak, max(0, s3+z)));  % min(maxPeak,max(0,s3+z));
